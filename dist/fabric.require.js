...
(function(global) {
    ...
    fabric.CustomPath = fabric.util.createClass(fabric.Path, {
        ...
        getSVGPathElement: function() {
            ...
            } else return this._getSVGPathElement();
        },
...
(function(global) {
    ...
    fabric.PathText = fabric.util.createClass(fabric.Text, fabric.Observable, {
        type: "PathText",
        textPath: null,
        ...
        allowPathProp: {
            ...
            cornerColor: "rgba(186, 186, 186, 0.9)",
            cornerSize: 10,
        ...
        _renderTextBackground: function(ctx) {
            if (this.textPath == null) {
                this._renderTextBoxBackground(ctx);
                this._renderTextLinesBackground(ctx, this._textLines);
            }
        },
        _renderTextFill: function(ctx) {
            ...
            this._renderTextLines("fillText", ctx, this._textLines);
        },
        ...
        _renderTextStroke: function(ctx) {
            ...
            this._renderTextLines("strokeText", ctx, this._textLines);
            ctx.closePath();
            ctx.restore();
        },
        ...
        _getOrderOfWidestLines: function(ctx, textLines) {
            ...
            while (objectsToOrderByWidth.length > 0) {
                ...
                if (!foundWiderLine) order.push(this._boundaries.indexOf(current));
            }
            return order;
        },
        ...
        _renderTextLines: function(method, ctx, textLines) {
            var order = !this.textPath ? textLines.map(function(line) {
                return textLines.indexOf(line);
            }) : this._getOrderOfWidestLines(ctx, textLines);
            ...
            while (order.length > 0) {
                ...
                var summedLineHeight = lineHeights.slice(0, currentIndex + 1).reduce(function(a, b) {
                    return a + b;
                }, 0);
                this._renderTextLine(method, ctx, textLines[currentIndex], -this._getMaximumConsumedDistance() / 2, this._getTopOffset() + summedLineHeight, currentIndex);
            }
        },
        _getAngleOfTangentAtDistanceInDegrees: function(distance) {
            var angle = 0;
            if (this.textPath) {
                var leftDistance = distance - 75e-5, rightDistance = distance + 75e-5;
            ...
            if (this._boundaries.length > 0) {
                ...
                for (var i = 0, len = this._boundaries.length; i < len; i++) {
                    ...
                    if (!(current.width == null)) {
                        if (distance == null || current.width > distance) {
        ...
        _drawTextResidueIfNecessary: function(method, ctx, line, left, top, lineIndex) {
            if (this.wantTextPathResidue && this.textPath) {
                var residueTracker = this._boundaries[lineIndex].residueHasBeenDrawn, residueTrackerIsUndefined = residueTracker == null ? true : false, residueBoxNeedsToBeDrawn = lineIndex == 0 && (residueTrackerIsUndefined || !residueTracker["bounding-box"]) ? true : false, isFirstPass = residueTrackerIsUndefined || residueTracker[method] == null || !residueTracker[method] ? true : false;
                if (isFirstPass) {
        ...
        _getMinorBoundingBoxAroundPoint: function(point, top, halfWidthOfLetter, halfHeightOfLetter, angleInDegrees, reverseMode) {
            ...
            var x = !reverseMode ? point.x - halfWidthOfLetter : point.x + halfWidthOfLetter, x2 = !reverseMode ? point.x + halfWidthOfLetter : point.x - halfWidthOfLetter;
            var thisUntransformedUpperLeadingPoint = new fabric.Point(x, point.y + top - halfHeightOfLetter), thisUntransformedLowerLeadingPoint = new fabric.Point(x, point.y + top + halfHeightOfLetter), thisUntransformedLowerTrailingPoint = new fabric.Point(x2, point.y + top + halfHeightOfLetter), thisUntransformedUpperTrailingPoint = new fabric.Point(x2, point.y + top - halfHeightOfLetter);
            var radians = fabric.util.degreesToRadians(angleInDegrees);
            var thisMinorBoundingBox = [ fabric.util.rotatePoint(thisUntransformedUpperLeadingPoint, point, radians), fabric.util.rotatePoint(thisUntransformedLowerLeadingPoint, point, radians), fabric.util.rotatePoint(thisUntransformedLowerTrailingPoint, point, radians), fabric.util.rotatePoint(thisUntransformedUpperTrailingPoint, point, radians) ];
        ...
        _pointsShareOnlyOneEdge: function(boundingBox, otherBoundingBox) {
            ...
            for (var c = 0, len = boundingBox.length; c < len; c++) {
                var firstIndex = c % len, secondIndex = (c + 1) % len, firstNotIndex = (c + 3) % len, secondNotIndex = (c + 2) % len;
        ...
        _getAcceptablePoint: function(runningDistance, widthOfCharacter, halfWidth, halfHeightOfLetter, halfNonTransformedHeight, top, previousPoint, reverseMode, wantLessTextPathOverlapFeature, letter) {
            ...
            var point, pass = 0, extraPassLimit = 10, pointIsNotAcceptable = false, thisDistancePlusHalfWidth = initialDistancePlusHalfWidth;
        ...
        _renderLetterAtPoint: function(method, ctx, letter, point, top, lineIndex, halfNonTransformedHeight, summedLineHeight, style, approximating) {
            if (point && this.wantTextPathWithLessOverlap && !point.outOfPath || point && this.removeExtraChar && !point.outOfPath) {
                ...
                var halfWidth = point.halfWidth, halfHeightOfLetter = point.halfHeightOfLetter;
        ...
        _calculateTextLineOnTextPath: function(method, ctx, line, left, top, lineIndex, approximating, spaceWidth) {
            ...
            if (this.textAlign === "center") {
                ...
                startingDistance = maximumConsumedDistance / 2 - this.width / 2;
        ...
        _pushStyleToContext: function(ctx, styleDeclaration) {
            ...
            var fill = styleDeclaration.fill || this.fill;
            ctx.fillStyle = fill == null ? fill : fill.toLive ? fill.toLive(ctx) : fill;
        ...
        _wrapSVGTextAndBg: function(markup, textAndBg, shadowSpans, offsets) {
            ...
            if (!(this.textPath == null)) {
                ...
                markup.push("<defs>\n", '<path id="' + textPathId, '" d="' + this.textPath.getSVGData(), '" style="', this.textPath.getSvgStyles(), '" transform="', this.textPath.getSvgTransform(), addTransform, this.textPath.getSvgTransformMatrix(), '" stroke-linecap="round" ', "/>\n", "</defs>\n");
            }
            var hasTextPathId = !(textPathId == null) ? true : false, svgTransformValue = [ this.getSvgTransform(), this.getSvgTransformMatrix() ].join(""), svgTransformAttribute = svgTransformValue !== "" ? ' transform="' + svgTransformValue + '"' : "", textLeft = !hasTextPathId ? offsets.textLeft : 0, textPathStartOffsetPercent = !hasTextPathId || this.textPathDistanceOffset == null ? 0 : 100 * this.textPathDistanceOffset / this.textPath.pathLength() || 0;
            markup.push("<g>\n", textAndBg.textBgRects.join(""), "<text ", this.fontFamily ? 'font-family="' + this.fontFamily.replace(/"/g, "'") + '" ' : "", this.originalFontSize ? 'font-size="' + this.originalFontSize + '" ' : "", this.fontStyle ? 'font-style="' + this.fontStyle + '" ' : "", this.fontWeight ? 'font-weight="' + this.fontWeight + '" ' : "", this.textDecoration ? 'text-decoration="' + this.textDecoration + '" ' : "", 'style="', this.getSvgStyles(), '" ', 'transform="translate()">\n', hasTextPathId ? '<textPath xlink:href="#' + textPathId + '" startOffset="' + textPathStartOffsetPercent + '%">\n' : "", shadowSpans.join(""), textAndBg.textSpans.join(""), hasTextPathId ? "</textPath>\n" : "", "</text>\n", "</g>\n");
        },
        ...
        _setSVGTextLineShadow: function(textLine, lineIndex, shadowSpans, lineHeight, lineTopOffsetMultiplier) {
            var lineLeftOffset = this._boundaries && this._boundaries[lineIndex] ? this._boundaries[lineIndex].left - lineTopOffsetMultiplier : 0, xValue = lineLeftOffset + lineTopOffsetMultiplier + this.shadow.offsetX, yOrDeltaYAttributeName = lineIndex === 0 || this.useNative ? "y" : "dy", yOrDeltaYValue = this.useNative ? lineHeight * lineIndex - this.height / 2 + this.shadow.offsetY : lineHeight + (lineIndex === 0 ? this.shadow.offsetY : 0), shadowFillColor = this.shadow.color && typeof this.shadow.color === "string" ? new fabric.Color(this.shadow.color) : "";
            shadowSpans.push("<tspan", " ", 'x="', fabric.util.toFixed(xValue, 2), '"', " ", yOrDeltaYAttributeName, '="', fabric.util.toFixed(yOrDeltaYValue, 2), '"', " ", 'stroke-opacity="' + shadowFillColor.getAlpha() + '"', " ", this._getFillAttributes(this.shadow.color), ">", fabric.util.string.escapeXml(textLine), "</tspan>");
        },
        _setSVGTextLineText: function(textLine, i, textSpans, lineHeight, lineTopOffsetMultiplier) {
            ...
            if (this.textAlign == "left" || this.textAlign == "justify") {
                ...
                lineLeftOffset = this._lastBoundaries && this._lastBoundaries[i] ? toFixed(this._lastBoundaries[i].letters[i].point.runningDistanceAfter * this.scaleX, 2) : 0;
            } else if (this.textAlign == "center") {
                lineLeftOffset = this._lastBoundaries && this._lastBoundaries[i] ? toFixed((this._lastBoundaries[i].letters[i].point.runningDistanceAfter - this._lastBoundaries[i].letters[i].point.distanceToConsume) * this.scaleX, 2) : 0;
            }
            textSpans.push('<tspan dx="', lineLeftOffset, '" ', i === 0 || this.useNative ? "y" : "dy", '="', toFixed(this.useNative ? lineHeight * i - this.height / 2 : lineHeight * lineTopOffsetMultiplier, 2), '" ', this._getFillAttributes(this.fill), ">", fabric.util.string.escapeXml(textLine), "</tspan>");
        },
        _getFillAttributes: function(value) {
            var fillColor = value && typeof value === "string" ? new fabric.Color(value) : "";
            if (!fillColor || !fillColor.getSource() || fillColor.getAlpha() === 1) {
...
(function(global) {
    ...
    fabric.CircleText = fabric.util.createClass(fabric.PathText, fabric.Observable, {
        type: "CircleText",
        radius: 150,
        startAngle: 0,
        endAngle: 360,
        ...
        _filterData: function(radius, startAngle, endAngle, clockwise) {
            ...
            return {
                ...
                endAngle: endAngle,
                lastCharToConcat: lastCharToConcat
            };
        },
        _getPathString: function(xCenter, yCenter, radius, startAngle, endAngle, clockwise) {
            ...
            var radius = pathInput.radius, startAngle = pathInput.startAngle, endAngle = pathInput.endAngle, lastCharToConcat = pathInput.lastCharToConcat;
            ...
            for (var curve in curves) {
                ...
                if (!pathData) pathData = "M " + (curve.x1 + xCenter) + " " + (curve.y1 + yCenter);
                pathData += " C " + (curve.x2 + xCenter) + " " + (curve.y2 + yCenter) + " " + (curve.x3 + xCenter) + " " + (curve.y3 + yCenter) + " " + (curve.x4 + xCenter) + " " + (curve.y4 + yCenter);
            }
        ...
        _createArc: function(radius, startAngle, endAngle) {
            var twoPI = Math.PI * 2;
            startAngle = startAngle % twoPI, endAngle = endAngle % twoPI;
            var curves = [];
